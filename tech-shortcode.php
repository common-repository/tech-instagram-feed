...
function tech_custom_js() {
 ...
 //print_r($tech_settings);
  $user_id = $tech_settings['tech_insta_user_id'];
  $access_token  = $tech_settings['tech_insta_access_token'];
  ...
  $FeedPadding = $tech_settings['tech_feed_padding'];
  $FeddPaddingUnit = $tech_settings['tech_feed_padding_unit'];
  $ShowLoadMore = $tech_settings['tech_load_more_button'];
 ...
 foreach($user_id as $user){
  ...
  // get user id from user name 
	...
	// get user media 
		$TechUserMedia = TechInstagramData("https://api.instagram.com/v1/users/$user/media/recent/?access_token=$access_token&count=$FeedCount");
	...
	$resultuserdata = TechInstagramData("https://api.instagram.com/v1/users/$user/?access_token=$access_token");
...
<?php	
		...
		if($TechUserMedia == ''){
			echo "<div>No Posts to show... Plz check your General Setting Details And Internet Connection. </div>";
  ...
  foreach ($TechUserMedia as $post) {
	  ...
	  ?>
       <div style="padding:<?php echo $FeedPadding . $FeddPaddingUnit;?>;" class="instagram-unit <?php echo $MediaWidth; ?>"><a  target="blank" href="<?php echo $post->link ?>">
  ...
  <input type="button" class="" onClick="InstaPagination()" value='<?php echo $LodMoreButtonText; ?>'>
  </div>
  </div></div>
 ...
 $InstaSettings = array(
		...
		"MediaPadding" => $FeedPadding,
		"MediaPaddingUnit" => $FeddPaddingUnit
	  );
