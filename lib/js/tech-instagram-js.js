...
function InstaPagination()
{
	...
	var MediaPadding = TechInstagram.MediaPadding;
	var MediaPaddingUnit = TechInstagram.MediaPaddingUnit;
	...
	if(LoadMoreDisable == k){
		jQuery('.loadbutton').css("visibility", "hidden");	
		}
	...
	jQuery.each(feed, function(index){
		...
		var InstaMedia = '<div style="padding:'+MediaPadding+''+MediaPaddingUnit+';" class="instagram-unit '+MediaWidth+'"><a  target="blank" href="'+post_image_link+'"><img src="'+post_image_src+'" alt="'+caption_text+'" /></a></div>';
